...
export default async function walletRoutes(fastify: FastifyInstance) {
  fastify.post('/add-credit', async (request, reply) => {
    const body = request.body as any
    const rawAmount = Number(body?.amount || Number(body?.credit_amount || 0) / 100)
    ...
    try {
      ...
      if (!pix.transaction_id) {
        return reply.status(502).send({ success: false, message: 'Resposta de deposito sem transacao.', data: payload })
      }
  ...
  fastify.get('/deposit-status', async (request, reply) => {
    const query = request.query as any
    const transactionId = String(query?.transaction_id || query?.transactionId || '')
    if (!transactionId) {
    ...
    try {
      ...
      return { success: res.ok, status, data: payload?.data || payload }
    } catch (err: any) {
      fastify.log.error(err)
